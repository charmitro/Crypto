////////////////////////////////////////////////////
// @INCOMPLETE: Add a drop-down menu for changing //
// encrpypt methods.                              //
////////////////////////////////////////////////////
#include "crypto.h"
...
Absolute::Absolute(const wxString &title)
    ...
    // ENCRYPTION BUTTON
    encb = new wxButton(this, BUTTON1, wxT("ENCRYPT"), wxPoint(175, 110),
                        wxSize(350, 30), 0);
    encb->SetBackgroundColour(*wxBLACK);
    encb->SetForegroundColour(*wxYELLOW);
    // DECRYPTION BUTTON
    decb = new wxButton(this, BUTTON2, wxT("DECRYPT"), wxPoint(175, 250),
                        wxSize(350, 30), wxOK, wxDefaultValidator,
                        wxButtonNameStr);
    decb->SetBackgroundColour(*wxBLACK);
    decb->SetForegroundColour(*wxYELLOW);
    // DIFFICULTY BUTTON
    difb = new wxButton(this, BUTTON3, wxT(""), wxPoint(52, 10), wxSize(20, 20),
                        wxOK, wxDefaultValidator, wxButtonNameStr);
    difb->SetBackgroundColour(*wxBLACK);

    // ENCRYPT STRING BOX
    enc = new wxTextCtrl(this, -1, wxT("Enter string to encrypt..."),
                         wxPoint(175, 5), wxSize(350, 100), wxTE_MULTILINE);
    // DECRYPT STRING BOX
    dec = new wxTextCtrl(this, -1, wxT(""), wxPoint(175, 145), wxSize(350, 100),
                         wxTE_MULTILINE);
    // KEY DIFFICULTY W/ LABEL
    diflabel =
        new wxStaticText(this, -1, "KEY: ", wxPoint(0, 10), wxSize(30, 20));
    difbox = new wxTextCtrl(this, -1, _T("2"), wxPoint(31, 10), wxSize(20, 20),
                            wxTE_MULTILINE);
    // SHA RESULT STRING BOX W/ LABEL
    shalabel =
        new wxStaticText(this, -1, "SHA256 HASHCODE FOR THE NONECRYPTED TEXT",
                         wxPoint(183, 280), wxSize(350, 50));
    shabox = new wxTextCtrl(this, -1, _T(""), wxPoint(175, 300),
                            wxSize(350, 50), wxTE_MULTILINE);
}
...
void Absolute::OnPressE(wxCommandEvent &event) {
    ...
    std::string s = std::string(peos.mb_str());
    int n = s.length();
    char text[n];
...
void Absolute::OnPressD(wxCommandEvent &event) {
    ...
    std::string s = std::string(mouni.mb_str());
    int n = s.length();
    char text[n];
...
void Absolute::OnAbout(wxCommandEvent &event) {
    wxMessageBox(
        wxT("This program allows you to encrypt string and also produce its"
            "hash code.\n\n Made by Charalampos Mitrodimas."),
        wxT("Crypto v1.0"));
}
// EXIT
void Absolute::OnExit(wxCommandEvent &event) { Close(true); }
